...
results_all$term <- results_all$term %>%
  gsub("treatmentbiogas", "BD", .) %>%
  gsub("treatmentcow", "CM", .) %>%
  gsub("treatmentpig", "PS", .)
...
if(!is.null(results_all) && nrow(results_all) > 0) {
  p2 <- results_all %>%
    ...
    theme(
      ...
      axis.text.y = element_text(size = 9), # Make gtdb-term labels italic
...
figure3ab <- ggarrange(p.genus, p2, ncol = 2, nrow = 1, align = "hv", widths =c(1.3, 1))
figure3ab

ggsave(plot = figure3ab, "figure3ab.jpg",
